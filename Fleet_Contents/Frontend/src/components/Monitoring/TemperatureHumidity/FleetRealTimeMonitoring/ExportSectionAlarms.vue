...
<template>
  <div class="export-section" :class="{ 'export-section--disabled': !props.enabled }">
    <div class="row items-center q-mb-sm">
      ...
      />
      <div class="q-ml-xs">
        <div class="text-subtitle2 text-weight-bold">Alarm Events</div>
        <div class="text-caption text-grey-6">
          Triggered alarms within date range (up to 200 records)
        </div>
      </div>
    </div>
    <div v-if="props.enabled" class="row q-col-gutter-sm q-ml-md">
      ...
      <div class="col-12">
        <div class="text-caption text-grey-5">Up to 200 alarm records. Times are UTC.</div>
      </div>
    </div>
  </div>
</template>
